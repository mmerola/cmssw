...
in_path = "/dpm/na.infn.it/home/cms/store/user/oiorio/2012/Fall12/MergedNov/"
out_path = "/cms/store/user/mmerola/SingleTop/Analysis/2012/Merged/"
#path = "/dpm/na.infn.it/home/cms/store/user/merola/SingleTop/DQM/"

dir ='"srm://'+in_se + ":" + in_port + "/srm/managerv2?SFN=" + in_path +'"'
...
for channel in channels:
    if "Merged" in channel:
        command_ls_channel = command_ls[:-1] +'"'
    else:    
    ...
    for file in files:
        if "Merged" in channel:
            ...
            filename = str(re.findall(".*\.root" , file))[(int(len(in_path))+int(len(channel))-3):-2]
        ...
        if "Merged" in channel:
            if nstart % nSimultaneous ==0 :
                command_cp = 'lcg-cp -b -D srmv2 -T srmv2 "srm://'+ in_se + ":" + in_port + "/srm/managerv2?SFN=" + file + '" "'+ 'srm://'+out_se + ":"+ out_port + "/srm/managerv2?SFN=" + out_path + '/' + filename +'"'
            else:
                command_cp = 'lcg-cp -b -D srmv2 -T srmv2 "srm://'+ in_se + ":" + in_port + "/srm/managerv2?SFN=" + file + '" "'+ 'srm://'+out_se + ":" + out_port +"/srm/managerv2?SFN=" + out_path + '/'  + filename +'" &'

        else:
            if nstart % nSimultaneous ==0 :
                command_cp = 'lcg-cp -b -D srmv2 -T srmv2 "srm://'+ in_se + ":" + in_port + "/srm/managerv2?SFN=" + file + '" "'+ 'srm://'+out_se + ":"+ out_port + "/srm/managerv2?SFN=" + out_path + '/' + channel + '/' + filename +'"'
            else:
                command_cp = 'lcg-cp -b -D srmv2 -T srmv2 "srm://'+in_se + ":" + in_port + "/srm/managerv2?SFN=" + file + '" "'+ 'srm://'+out_se + ":" + out_port +"/srm/managerv2?SFN=" + out_path + '/' + channel + '/' + filename +'" &'
