...
fName = "copyFlavorSeparationTemplateSummerOLD.py"
...
def appendInput(fileName,directory,channel,prefix,parts): 
    ...
    inputRedirect = "ls -ltr "+directory +"/"+channel+"/ | grep " + prefix + " | cut -c46-150 > "+ channel+"_input.py"
    if channel == "Data" or "QCDEle" in directory or "V2" in directory:
        inputRedirect = "ls -ltr "+directory +"| grep " + prefix + " | cut  -c46-150 > "+ channel+"_input.py"
...
def haddInput(directory,channel,prefix): 
    ...
    inputRedirect = "ls -ltr "+directory +"/"+channel+"/ | cut -c46-150 > "+ channel+"_input.py"
    if channel == "Data":
        inputRedirect = "ls -ltr "+directory +"| cut -c46-150 > "+ channel+"_input.py"
    #if "HT_" in channel or "QCD_Pt" in channel:
...
for channel in channels:
    ...
    command = 'nohup cmsRun ./' + channel+SuffixCfg+' > /tmp/mmerola/'+channel+'_merge.log &'
    command_bsub = 'bsub -q1nd ' + channel + SuffixBsub
    command_hadd = 'nohup ./'+channel+'_hadd.py > /tmp/mmerola/'+channel+'_hadd.log &' 
